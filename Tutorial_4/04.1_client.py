    ...
    def _recibir_msgs(self):
        ...
        while self.activo:
            try:
                ...
                if not data:
                    self.log(f"[Cam {self.cam_index}] Conexión cerrada por servidor")
                    break
                
                # Agregar datos al buffer y procesar líneas completas
                ...
                while "\n" in data_buffer:
                    ...
                    if msg.startswith("SERIAL:"):
                        cmd = msg[7:]
                        self.log(f"[Servidor → robot] {cmd}")
                        self.app.manejar_serial(cmd)
                    elif msg.startswith("< "):
                        # Mensajes de chat / texto del servidor
                        self.log(f"[Servidor] {msg[2:].strip()}")
                    else:
                        # Mensajes directos que deben ir al robot, p.ej. 111 o qqq
                        self.log(f"[Servidor → robot] {msg}")
                        self.app.manejar_serial(msg)
            except Exception as e:
                self.log(f"[Cam {self.cam_index}] Recibir error: {e} [{type(e).__name__}]")
                break
        self.activo = False
...
class ClienteCamara:
    ...
    def _serial_send(self, cmd_bytes: bytes):
        with self._serial_lock:
            if not self.serial_port.is_open:
                try:
                    texto = cmd_bytes.decode('utf-8', errors='ignore').strip()
                except Exception:
                    texto = repr(cmd_bytes)
                self.log(f"IGNORADO (serial cerrado): {texto}")
                return
            
            try:
                written = self.serial_port.write(cmd_bytes)
                try:
                ...
                try:
                    ...
                    texto = repr(cmd_bytes)
                self.log(f"Serial > {texto} (bytes={written})")
            except Exception as e:
                self.log(f"Error serial: {e} [{type(e).__name__}]")

    def manejar_serial(self, cmd: str):
        """
        Recibe comandos del servidor y los reenvía al puerto serial del robot.
        Aplica debounce para evitar envíos duplicados en corto tiempo.
        """
        if cmd == "ON":
        ...
        ahora = time.time()
        if ahora - self._serial_cmd_ts.get(cmd_limpio, 0) < 0.1:
            return  # Debounce: ignore si fue hace poco
